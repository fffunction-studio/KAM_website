...
class FietsenSwiper {
  ...
  init() {
    ...
    swipers.forEach(fietsSwiper => {
      let slidesPerView = fietsSwiper.container.getAttribute('data-slides-per-view') ? container.getAttribute('data-slides-per-view') : 1
      let spaceBetween = fietsSwiper.container.getAttribute('data-space-between') ? container.getAttribute('data-space-between') * 4 : 0
      let freeMode = fietsSwiper.container.getAttribute('data-free-mode') ? container.getAttribute('data-free-mode') == 'true' : false
      let autoplay = fietsSwiper.container.getAttribute('data-autoplay') ? container.getAttribute('data-autoplay') == 'true' : false
